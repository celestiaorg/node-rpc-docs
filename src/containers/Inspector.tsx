...
const useCustomTransportList = createPersistedState(
  "inspector-custom-transports",
);
...
const Inspector: React.FC<IProps> = (props) => {
  ...
  useEffect(() => {
    if (selectedTransport !== undefined) {
      ...
      const s: IWebTransport = selectedTransport;
  ...
  useEffect(() => {
    if (props.transport) {
      const t = transportList.find(
        (tp: ITransport) =>
          tp.name?.toLowerCase() === props.transport?.toLowerCase(),
      );
      if (t) {
    ...
    <>
        ...
        <Container maxWidth="sm">
          ...
          {requestHistory.length === 0 ? (
            ...
            <Grid container style={{ paddingBottom: "30px" }}>
              ...
              >
                {requestHistory.map(
                  (requestHistoryItem: any, historyIndex: number) => (
                    <ListItem
                      button
                    ...
                    >
                      <ListItemText
                        primary={
                          requestHistoryItem.content.method || "Empty Method"
                        }
              ...
              <MonacoEditor
                ...
                value={
                  requestHistory[historySelectedIndex]
                    ? JSON.stringify(
                        requestHistory[historySelectedIndex].content,
                        null,
                        4,
                      )
                    : ""
                ...
                <div style={{ userSelect: "none" }}>
                  ...
                  {tabIndex === index ? (
                    <Tooltip title="Close Tab">
                      <IconButton
                        onClick={(ev) => handleClose(ev, index)}
                        style={{
                          position: "absolute",
                          right: "10px",
                          top: "25%",
        ...
        <Toolbar>
          ...
          <TransportDropdown
            ...
            onAddTransport={(addedTransport: ITransport) => {
              setTransportList([...transportList, addedTransport]);
            }}
            selectedTransport={selectedTransport}
            onChange={(changedTransport) =>
              setSelectedTransport(changedTransport)
            }
            style={{
              marginLeft: "10px",
            }}
                    ...
                    title={
                      <div style={{ textAlign: "center" }}>
                        ...
                        <Typography variant="caption">
                          A JSON-RPC endpoint may respond to the rpc.discover
                          method or a provide a static document. This adds
                          features like auto completion to the inspector.
                        </Typography>
                      </div>
                    }
                    onClick={() =>
      ...
      >
        <SplitPane
          ...
          minSize={100}
          maxSize={-100}
          defaultSize={selectedTransport.schema ? "85%" : "100%"}
          pane2Style={{ height: "100%", width: "100%", overflow: "auto" }}
          style={{ flexGrow: 1, height: "calc(100% - 128px)" }}
        >
          ...
          />
          {selectedTransport.schema && (
            <OptionsEditor
              schema={selectedTransport.schema}
