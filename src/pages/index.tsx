...
const Inspector = dynamic(() => import('@/containers/Inspector'), {
  ssr: false,
})
...
export default function Example() {
  ...
  return (
    <>
      ...
      <Transition.Root show={openTypeModal} as={Fragment}>
        <Dialog as='div' className='relative z-10' onClose={setOpenTypeModal}>
          <Transition.Child
            as={Fragment}
      ...
      <Transition.Root show={openInspectorModal} as={Fragment}>
        <Dialog
          as='div'
          className='relative z-10'
          onClose={setOpenInspectorModal}
        >
          <Transition.Child
            as={Fragment}
            enter='ease-out duration-300'
                ...
                <Dialog.Panel className='relative transform rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6'>
                  <div className='absolute top-0 right-0 hidden pt-4 pr-4 sm:block'>
                    <button
                      type='button'
                      className='rounded-md bg-white text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2'
                      onClick={() => setOpenInspectorModal(false)}
                    >
                      <span className='sr-only'>Close</span>
                      <XMarkIcon className='h-6 w-6' aria-hidden='true' />
                    </button>
