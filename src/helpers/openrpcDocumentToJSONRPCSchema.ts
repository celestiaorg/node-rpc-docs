...
const openrpcDocumentToJSONRPCSchema = (openrpcDocument: OpenrpcDocument) => {
  return {
    ...
    properties: {
      ...
      method: {
        ...
        oneOf:
          ...
          openrpcDocument.methods.map((method_a) => {
            const method = method_a as MethodObject;
            return {
              const: method.name,
    ...
    allOf:
      openrpcDocument &&
      openrpcDocument.methods &&
      openrpcDocument.methods.map((methoda) => {
        const method = methoda as MethodObject;
        return {
          if: {
            properties: {
              method: {
                ...
                oneOf: [
                  {
                    ...
                    minItems:
                      method.params &&
                      method.params.filter((param: any) => param.required)
                        .length,
